`ifndef CFS_MD_AGENT_SLAVE_SV
    ...
    class cfs_md_agent_slave#(int unsigned DATA_WIDTH = 32) 
        extends cfs_md_agent#(.DATA_WIDTH(DATA_WIDTH), .ITEM_DRV(cfs_md_item_drv_slave));
        ...
        function new(string name = "", uvm_component parent);
            ...
            cfs_md_driver#(DATA_WIDTH, cfs_md_item_drv_slave)::type_id::set_inst_override(
                cfs_md_driver_slave#(DATA_WIDTH)::get_type(),
                "driver",
                this
            );

            cfs_md_sequencer_base#(cfs_md_item_drv_slave)::type_id::set_inst_override(
                cfs_md_sequencer_slave#(DATA_WIDTH)::get_type(),
                "sequencer",
                this
            );
        ...
        protected virtual function void connect_port_from_mon_to_slave_seq();
            ...
            if (agent_config.get_active_passive() == UVM_ACTIVE) begin
                if ($cast(sequencer, super.sequencer) == 0) begin
                    `uvm_fatal(
                        "ALGORITHM_ISSUE",
                        $sformatf(
                            "Could not cast %0s to %0s.",
                            super.sequencer.get_full_name(), cfs_md_sequencer_slave#(DATA_WIDTH)::type_id::type_name
                        ) 
                    )
                end
        ...
        virtual function void connect_phase(uvm_phase phase);
            ...
            connect_port_from_mon_to_slave_seq();
        endfunction  
