`ifndef CFS_MD_DRIVER_SLAVE_SV
    ...
    class cfs_md_driver_slave#(int unsigned DATA_WIDTH = 32) 
        extends cfs_md_driver#(.DATA_WIDTH(DATA_WIDTH), .ITEM_DRV(cfs_md_item_drv_slave));

        typedef virtual cfs_md_if#(DATA_WIDTH) cfs_md_vif;

        `uvm_component_param_utils(cfs_md_driver_slave#(DATA_WIDTH))

        // Variable hiding of the cfs_md_agent_config in cfs_md_agent
        ...
        virtual function void end_of_elaboration_phase(uvm_phase phase);
            ...
            if (super.agent_config == null) begin
                `uvm_fatal("ALGORITHM_ISSUE", $sformatf("At this point the pointer to agent_config from %0s should not be null", 
                    get_full_name()))
            end

            if ($cast(this.agent_config, super.agent_config) == 0) begin
        ...
        protected virtual task drive_transaction(cfs_md_item_drv_slave item);
            ...
            // Ελέγχουμε εάν έχει οδηγηθεί item από τον master πρώτα
            if (vif.valid !== 1) begin
                `uvm_error("ALGORITHM_ISSUE", $sformatf("Trying to drive an item from slave when no item has been drived by the master - item: %0s", 
                    item.convert2string()))
            end
